...
// Get nearby users for the authenticated user (GUEST can see nearby PROFESSIONALS)
const getMyNearbyUsers = catchAsync(
  async (req: Request & { user?: any }, res) => {
    ...
    const data = await DistanceService.findNearbyUsers({
      userId: req.user.id,
      radiusKm,
      role: "PROFESSIONAL", // Fixed to PROFESSIONAL for GUEST users
      limit,
    });
    ...
    sendResponse(res, {
      statusCode: httpStatus.OK,
      success: true,
      message: `Found ${data.length} services from nearby professionals`,
      data: {
        services: data,
        count: data.length,
