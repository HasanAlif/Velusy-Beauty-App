...
const getCompletedBookingDetails = catchAsync(async (req, res) => {
  const userId = req.user?._id || req.user?.id || req.user?.userId;
  const { bookingId } = req.params;

  const booking = await bookingService.getCompletedBookingDetails(
    userId,
    bookingId
  );
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: "Completed booking details retrieved successfully",
    data: booking,
  });
});
...
const getRejectedBookingsDetails = catchAsync(async (req, res) => {
  const userId = req.user?._id || req.user?.id || req.user?.userId;
  const { bookingId } = req.params;

  const booking = await bookingService.getRejectedBookingsDetails(
    userId,
    bookingId
  );
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: "Rejected booking details retrieved successfully",
    data: booking,
  });
});
